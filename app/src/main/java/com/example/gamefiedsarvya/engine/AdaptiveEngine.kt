...
object AdaptiveEngine {
    ...
    // ── Hardware / Camera Focus Integration ──────────────────────────────────

    /**
     * Updates the DigitalTwin with real focus/engagement signals from the
     * FocusDetector (ML Kit face detection). Call this after each FocusResult.
     */
    fun updateFocusSignals(twin: DigitalTwin, focusScore: Float, engagementScore: Float): DigitalTwin {
        // Smooth with existing values (EMA, alpha=0.3)
        val newFocus      = twin.focusScore      * 0.7f + focusScore      * 0.3f
        val newEngagement = twin.engagementScore * 0.7f + engagementScore * 0.3f
        return twin.copy(
            focusScore      = newFocus.coerceIn(0f, 1f),
            engagementScore = newEngagement.coerceIn(0f, 1f)
        )
    }
